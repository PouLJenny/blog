...
解的情况
1. 当 $a \gt 0$ 时， $x \lt \frac{b}{a}$
2. 当 $a \lt 0$ 时， $x \gt \frac{b}{a}$
3. 当 $a=0,b\gt0$时 ,$x \in \R$
4. 当 $a=0,b\le0$ 时，$x \in \empty$
...
1. 分别糗事不等式组中各个不等式的解集
2. 求哥哥解集的公共部分，复杂情况下可借助数轴


### 一元二次不等式

形如 $ax^2+bs+c\gt0,(\ge0 , or \lt0 , or \le0)$的不等式称为一元二次不等式。一元二次不等式的最终要的解法就是结合一元二次函数 $f(x)=ax^2+bx+c$的图像来求解
...
设$x_1,x_2,x_3, \cdots,x_n$为$n$个实数，称$\frac{x_1 + x_2 + x_3+ \cdots+x_n}{n}$为算术平均值

#### 几何平均值

设$x_1,x_2,x_3, \cdots,x_n$为$n$个正实数，称$\sqrt[n]{x_1x_2x_3 \cdots x_n}$为几何平均值

#### 方差

设$x_1,x_2,x_3, \cdots,x_n$为$n$个实数


### 直线的斜率

直线的斜率表示直线的倾斜程度，通常定义为直线上两点的纵坐标差与横坐标差之间的比值

对于通过两点$A(x_1,y_1)$和$B(x_2,y_2)$的直线，其斜率$m$定义为:
$$
m = \frac{y_2-y_1}{x_2-x_1}
$$

斜率的几何意义：
1. 正斜率：如果$m \gt 0$,表示直线从左到右上升
1. 负斜率：如果$m \lt 0$,表示直线从左到右下降
1. 零斜率：如果$m = 0$,表示直线与$x$轴水平
1. 无穷大斜率: 对于垂直$x$轴的直线，斜率不存在或称为无穷大

### 直线方程的几种表示形式

1. 一般式,适用所有直线 $Ax+By+C=0$
2. 点斜式,
   已知直线上的一点$P(x_0,y_0)$,并且直线的斜率$k$存在，则直线可表示为$y-y_0=k(x-x_0)$,当$k$不存在时，直线可表示为$x=x_0$
3. 斜截式
   在$y$轴上截距为$b$,即过点$(0,b)$,斜率为$k$的直线,由点斜式可得斜截式$y=kx+b$;当直线的斜率不存在时不能使用斜截式表示
4. 截距式
   适用于与坐标轴不垂直的直线，已知直线与$x$轴相交与点$(a,0)$,与$y$轴相交与点$(0,b)$,则直线可表示为$bx + ay - ab =0$。特别地，当$a,b$ 均不为0时，截距式可写为$\frac{x}{a} + \frac{y}{b} = 1$
5. 两点式
   设直线过$A_1(x_1,y_1),A_2(x_2,y_2)$,$(x_1 \ne x_2 且 y_1 \ne y_2)$，则直线的方程为$\frac{y-y_1}{y_2-y_1} = \frac{x-x_1}{x_2-x_1}$
